...
def main(unused_argv):
  ...
  if eval_type == 'rgb':
    ...
    model_input = tf.placeholder(tf.float32,
        shape=(batch_size, None, _IMAGE_SIZE, _IMAGE_SIZE, 3))
    y = tf.placeholder(tf.int32, batch_size)
    with tf.variable_scope('RGB'):
      rgb_model = i3d.InceptionI3d(_NUM_CLASSES, spatial_squeeze=False, final_endpoint='Mixed_5c')
  ...
  if eval_type == 'flow':
    ...
    model_input = tf.placeholder(tf.float32,
        shape=(batch_size, None, _IMAGE_SIZE, _IMAGE_SIZE, 2))
    y = tf.placeholder(tf.int32, batch_size)
  ...
  with tf.Session(config=config) as sess:
    ...
    utils.ensure_dir('../results/checkpoints/{}4/model_firstckpt_{}'.format(dataset,eval_type))
    save_path = model_saver.save(sess,'../results/checkpoints/{}4/model_firstckpt_{}/model.ckpt'.format(dataset,eval_type))
