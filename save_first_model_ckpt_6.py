...
def main(unused_argv):
  ...
  if eval_type == 'rgb':
    ...
    with tf.variable_scope('RGB'):
      ...
      with tf.variable_scope(('inception_i3d/'+ end_point)):
        ...
        bn = snt.BatchNorm()
        rgb_logits = bn(rgb_net, is_training=is_training, test_local_stats=False)
  ...
  if eval_type == 'flow':
    ...
    with tf.variable_scope('Flow'):
      ...
      print( '===flow_net.shape:', flow_net.shape)
      # Adding a replacement logits layer
      end_point = 'Logits'
      with tf.variable_scope('inception_i3d/'+ end_point):
  ...
  with tf.Session(config=config) as sess:
    ...
    utils.ensure_dir('../results/checkpoints/{}6/model_firstckpt_{}'.format(dataset,eval_type))
    save_path = model_saver.save(sess,'../results/checkpoints/{}6/model_firstckpt_{}/model.ckpt'.format(dataset,eval_type))
